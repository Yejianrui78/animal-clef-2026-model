...
#   'fusion_salamander_only' — 仅 SalamanderID2025 做 Mega‖Miew；海龟(SeaTurtle)→Mega，猞猁/角蜥→Miew
...
FEATURE_STRATEGY = 'fusion_salamander_only'  # 'fusion' | 'fusion_salamander_only' | ...

# 只跑部分子集时设置（名称须与 metadata.csv 里 dataset 列一致，对应如 images/SalamanderID2025）。
# None 表示四个子集全跑；正式提交前请改为 None。
RUN_SUBSETS_ONLY =['LynxID2025']
# ============================================================
...
FORCED_CLUSTER_METHOD = 'hdbscan'  # 'dbscan' | 'hdbscan' | 'agglomerative'
...
def run_Agglomerative(similarity: np.ndarray, distance_threshold: float) -> np.ndarray:
    ...
    return np.asarray(clf.fit_predict(dist), dtype=int)
...
def extract_features_pipeline(
    ...
    *,
    ...
    if use_fusion:
        return extract_fused_features(mega_model, miew_model, dataset, tta=use_tta, dataset_name=dataset_name)
    if single_backbone == 'mega':
        return extract_features_single(mega_model, dataset, size=384, tta=use_tta, dataset_name=dataset_name)
    if single_backbone == 'miew':
        return extract_features_single(miew_model, dataset, size=512, tta=use_tta, dataset_name=dataset_name)
...
def search_best_agglo_threshold(
    ...
    for t in threshold_grid:
        pred = run_Agglomerative(similarity, t)
        ari = adjusted_rand_score(true_labels, pred)
        if ari > best_ari:
...
if __name__ == '__main__':
    ...
    if per_dataset_bb:
        ...
        print("    （仅 SalamanderID2025 双模型融合；海龟→Mega，猞猁/角蜥→Miew）")
    ...
    if FEATURE_STRATEGY == 'fusion':
        ...
        feat_desc = '仅 SalamanderID2025 Mega‖Miew；海龟→Mega，猞猁/角蜥→Miew'
    elif per_dataset_bb:
    ...
    for name in all_names:
        ...
        if GRID_SEARCH_AGGLOMERATIVE:
            best_thr, best_ari_agglo = search_best_agglo_threshold(train_sim, true_labels)
            print(
    ...
    for name in all_names:
        ...
        if p['method'] == 'hdbscan':
            ...
            clusters = run_Agglomerative(test_sim, p['param'])
        else:
